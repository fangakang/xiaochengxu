...
Page({
  ...
  invite:function(){
    if (this.data.isJoin.id){
      this.setData({
        gdid: this.data.isJoin.id
      })
    }
    wx.navigateTo({
      url: 'invite?gdid='+this.data.gdid,
    })
  },
  goodShow:function(){
    this.setData({
  ...
  cutMoney: function (e) {//我要砍
    ...
    if (this.data.barDetail.flag == true) {
      ...
      wx.request({
        ...
        success: function (res) {
          var detailid=res.data.err_msg.id
          console.log(res)
          wx.request({
            url: app.globalData.urlHead +'/cashier/merchants.php?m=Api&c=wxadoc&a=userBargain',
            data: { 
  ...
  bargain:function(){
    ...
    wx.request({
      ...
      data: {
        ...
        openid: getApp().globalData.openid,
        appid: getApp().globalData.appid,
        shareUnionid: that.data.shareUnionid,
        is_store: 1,
        activeid: 5
      },
      method: 'POST',
      ...
      success: function (res) {
        ...
        that.setData({
          ...
          isJoin: res.data.err_msg.isJoin
        })
        that.countDown();
        //计算砍价进度
