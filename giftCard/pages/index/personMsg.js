...
Page({
  ...
  formSubmit:function(e){
    ...
    if (that.data.wx_coupon.payactive==2){//需支付激活
      ...
      wx.request({
        url: app.globalData.urlHead + '/cashier/merchants.php?m=Api&c=wxadoc&a=weixinPayCard',
        data: {
  ...
  activateCard:function(){
    ...
    wx.request({
      url: app.globalData.urlHead + '/cashier/merchants.php?m=Api&c=wxadoc&a=activateCard',
  ...
  getCityPicker: function () {
    var that = this;
    wx.request({
      url: app.globalData.urlHead +'/cashier/merchants.php?m=Index&c=index&a=getCityPicker',
      data: '',
      header: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
  ...
  showPersonInfo: function (data) {//显示会员信息
    ...
    wx.request({
      url: app.globalData.urlHead +'/cashier/merchants.php?m=Api&c=wxadoc&a=jumpPage',
      data: postData,
      header: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
